...
class Partie:
    ...
    def __init__(self, tapis=50, mise=10):
        """Initialisation du nombre de jetons (mise) par joueur et de leur tapis"""
        self.tapis = tapis
        self.mise = mise
    ...
    def verif(self, tapis_joueur):
        """Détermine si un joueur est participant ou non, complémentaire de la fonction jouer()"""
...
class Joueur:
    ...
    def __init__(self, tapis, nom):
        """Donne des atributs à la classe joueur (son nom, sa main, son tapis, la meilleure combinaison de sa main"""
        self.nom = nom
        self.main = []
        self.tapis = tapis
    ...
    def evaluer(self):
        """Détermine la meilleure combinaison de sa main"""
        self.combinaison = Combinaison(self.main)

    def nouvelle_main(self):
        """Réinitialise la main et la combinaison des joueurs"""
        self.main.clear()
        del self.combinaison
...
class Coup(Partie):
    ...
    def abattre(self):
        """Détermine le gagnant de ce coup"""
        main_joueurs = []
...
class Croupier(Coup):
    ...
    def rassembler(self):
        """Créer et instancie 52 cartes différentes dans un paquet"""
        for couleur in "PCKT":
    ...
    def distribuer(self, n):
        """Distribue 5 cartes à chaque joueur participant"""
        for i in range(len(self.joueurs_participants)):
            for j in range(n):
